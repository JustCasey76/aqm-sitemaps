...
|-----------|-------------|---------|---------------|
| folder_slug | Single folder to include in the sitemap | empty | "services" |
| folder_slugs | Multiple folders to include (comma-separated) | empty | "services,products,resources" |
| display_type | How to display the sitemap | columns | "columns", "inline" |
| columns | Number of columns (when display_type is "columns") | 2 | 1-6 |
| order | How to order the pages | menu_order | "menu_order", "title", "date" |
| exclude_ids | IDs of pages to exclude (comma-separated) | empty | "123,456,789" |
| exclude_current | Whether to exclude the current page from the sitemap | no | "yes", "no", "true", "false", "1", "0" |
| item_margin | Bottom margin for each list item | 10px | "10px", "0.5em", etc. |
| icon | Font Awesome icon class | empty | "fa fa-arrow-right" |
| icon_color | Color for the icon | empty | "#007bff", "red" |
| disable_links | Whether to disable links and show only titles | no | "yes", "no" |

### Example Shortcodes

```
[sitemap_page folder_slug="services" display_type="columns" columns="2" order="menu_order" icon="fas fa-check" icon_color="#007bff" item_margin="10px"]
```

```
[sitemap_page folder_slugs="services,resources" exclude_current="yes" display_type="inline"]
```
