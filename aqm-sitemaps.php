...
define('AQM_SITEMAPS_VERSION', '3.0.3');
...
/**
 * Check if the plugin needs to be reactivated after an update
 */
function aqm_sitemaps_check_reactivation() {
    $was_active = get_option('aqm_sitemaps_was_active', false);
    
    if ($was_active) {
        error_log('[AQM SITEMAPS] Plugin was active before update, reactivating');
        
        // Make sure we have the activation function
        ...
        activate_plugin($plugin_basename, '', is_network_admin(), true);
        
        // Clear the flag
        delete_option('aqm_sitemaps_was_active');
        
        error_log('[AQM SITEMAPS] Plugin reactivated successfully');
    }
}
                    ...
                    <form id="aqm-sitemap-form">
                        ...
                        <div class="form-grid">
                            ...
                            <div class="form-section content-settings">
                                ...
                                <div class="form-group">
                                    <label for="exclude_current">Exclude Current Page:</label>
                                    <select id="exclude_current" name="exclude_current">
                                        <option value="no">No</option>
                                        <option value="yes">Yes</option>
                                    </select>
                                    <div class="form-help">Automatically exclude the page where the sitemap is displayed</div>
                                </div>
                            </div>
...
function display_enhanced_page_sitemap($atts) {
    ...
    $atts = shortcode_atts(array(
        ...
        'exclude_current' => 'no', // Parameter to exclude the current page
    ...
    // Add current page ID to exclusion list if exclude_current is enabled
    if ($exclude_current) {
        global $post;
        if (isset($post) && is_object($post) && !empty($post->ID)) {
            $current_page_id = $post->ID;
            if (!in_array($current_page_id, $exclude_ids)) {
                $exclude_ids[] = $current_page_id;
            }
        }
    }
